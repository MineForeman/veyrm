...
void LoginScreen::handleRegistration() {
    ...
    // Validate inputs
    if (reg_username_input.empty() || reg_email_input.empty() ||
        reg_password_input.empty() || reg_confirm_password_input.empty()) {
    ...
    if (!isValidEmail(reg_email_input)) {
...
bool LoginScreen::isValidEmail(const std::string& email) const {
